  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-[1600px] mx-auto h-[calc(100vh-8rem)] pb-6">
      <div className="flex items-center gap-3 border-b border-border/50 pb-4">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-100">AI Assistant</h1>
      ...
      <div className="glass-card flex-1 flex flex-col overflow-hidden relative">
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {messages.map((msg) => (
            <div key={msg.id} className={cn("flex gap-4 max-w-2xl", msg.role === 'user' ? "ml-auto flex-row-reverse" : "")}>
              <div className={cn("w-8 h-8 rounded-full flex items-center justify-center shrink-0 mt-1", msg.role === 'user' ? "bg-surfaceHighlight" : "bg-primary/20 text-primary")}>
                {msg.role === 'user' ? <User className="w-4 h-4 text-gray-400" /> : <Bot className="w-4 h-4" />}
              </div>
              <div className={cn("p-4 rounded-2xl", msg.role === 'user' ? "bg-primary/20 border border-primary/30 text-gray-100" : "bg-surfaceHighlight border border-border/50 text-gray-300")}>
        ...
        <form onSubmit={handleSend} className="p-4 bg-surfaceHighlight/30 border-t border-border/50 flex gap-3">
          <input 
            type="text" 
            value={input}
            onChange={e => setInput(e.target.value)}
            placeholder="Ask me anything..."
            className="flex-1 bg-surface border border-border rounded-xl px-4 py-3 text-sm text-gray-100 focus:outline-none focus:border-primary transition-colors"
          />
